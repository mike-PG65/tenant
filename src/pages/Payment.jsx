...
export default function PaymentSection() {
  ...
  useEffect(() => {
    ...
    socket.on("connect", () => {
      socket.emit("registerTenant", tenantId); // join room with tenantId
    });
  ...
  const handleDownload = async () => {
    ...
    try {
      ...
      pdf.text(`© ${new Date().getFullYear()} Tenant Portal — All rights reserved`, pdf.internal.pageSize.getWidth() / 2, pdf.internal.pageSize.getHeight() - 10, { align: "center" });

      const date = new Date().toISOString().slice(0, 10);
      pdf.save(`Rent_Receipt_${payment?.tenantName || "Tenant"}_${payment?.month || date}.pdf`);
    } catch (err) {
  ...
  // ---------------- Derived state: has paid this month ----------------
  const hasPaidCurrentMonth = payment?.month === new Date().toISOString().slice(0, 7)
    && payment?.status === "successful";

  // ---------------- Render ----------------
  if (rentalLoading) return <p className="text-center text-gray-500 animate-pulse">Loading your rental details...</p>;
  if (!rental) return <p className="text-center text-red-600">No rental record found.</p>;

  // Show receipt if tenant has already paid this month
  if (hasPaidCurrentMonth) {
    return (
      <div ref={receiptRef} className="bg-gradient-to-b from-white to-blue-50 p-6 rounded-2xl shadow-lg max-w-2xl mx-auto">
  ...
  return (
    <div className="p-8 bg-white rounded-3xl shadow-2xl space-y-6 max-w-lg mx-auto">
      <h2 className="text-3xl font-bold text-gray-800 text-center">Complete Your Payment</h2>

      {status.message && (
        <div className={`p-3 rounded-lg text-center font-medium ${status.type === "success" ? "bg-green-100 text-green-700" :
